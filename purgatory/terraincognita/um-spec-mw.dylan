...
define constant <register-bank> = <vector>; //limited(<vector>, of: <machine-word>, size: 8);
define constant <scroll> = <vector>; //limited(<vector>, of: <machine-word>);
...
define class <universal-machine>(<object>)
  constant slot regs :: <register-bank> = make(<register-bank>, size: 8, fill: #x00000000);
...
define method initialize(um :: <universal-machine>, #key)
    ...
    um.scroll := read-scroll("C:/OD/Projects/UM-ICFP/codex.umz");
end;
...
*/
  ...
  let operator = as(<integer>, u%shift-right(platter, 32 - 4));

  local method A(platter :: <machine-word>) um.regs[as(<integer>, u%shift-right(%logand(platter, 7 * 64), 6))] end,
        method A-setter(new :: <machine-word>, platter :: <machine-word>) um.regs[as(<integer>, u%shift-right(%logand(platter, 7 * 64), 6))] := new end,
        method literal-A-setter(new :: <machine-word>, platter :: <machine-word>) um.regs[as(<integer>, %logand(u%shift-right(platter, 25), 7))] := new end,
        method B(platter :: <machine-word>) um.regs[as(<integer>, u%shift-right(%logand(platter, 7 * 8), 3))] end,
        method B-setter(new :: <machine-word>, platter :: <machine-word>) um.regs[as(<integer>, u%shift-right(%logand(platter, 7 * 8), 3))] := new end,
        method C(platter :: <machine-word>) um.regs[as(<integer>, %logand(platter, 7))] end,
        method C-setter(new :: <machine-word>, platter :: <machine-word>) um.regs[as(<integer>, %logand(platter, 7))] := new end,
        method get-array(i :: <integer>) if (i = 0) um.scroll else um.arrays[i] end if end;

/*
  local method A(platter :: <machine-word>) as(<integer>, um.regs[u%shift-right(%logand(platter, 7 * 64), 6)]) end,
        method A-setter(new :: <machine-word>, platter :: <machine-word>) um.regs[u%shift-right(%logand(platter, 7 * 64), 6)] := new end,
        method literal-A-setter(new :: <machine-word>, platter :: <machine-word>) um.regs[%logand(u%shift-right(platter, 25), 7)] := new end,
        method B(platter :: <machine-word>) as(<integer>, um.regs[u%shift-right(%logand(platter, 7 * 8), 3)]) end,
        method B-setter(new :: <machine-word>, platter :: <machine-word>) um.regs[u%shift-right(%logand(platter, 7 * 8), 3)] := new end,
        method C(platter :: <machine-word>) as(<integer>, um.regs[%logand(platter, 7)]) end,
        method C-setter(new :: <machine-word>, platter :: <machine-word>) um.regs[%logand(platter, 7)] := new end,
        method get-array(i :: <integer>) if (i = 0) um.scroll else um.arrays[i] end if end;
*/
  select (operator)
...
*/
    ...
    0 => unless (u%<(platter.C, 1)) platter.A := platter.B end unless;
...
*/
    2 => get-array(as(<integer>, platter.A))[as(<integer>, platter.B)] := platter.C;
...
*/
    3 => platter.A := u%+(platter.B, platter.C);
...
/*
           #4. Multiplication.
                  ...
                  the value in register C, modulo 2^32.
*/
    4 => platter.A := u%*(platter.B, platter.C);
...
/*
           #5. Division.
                  ...
                  bit number.
*/
    5 => platter.A := u%divide(platter.B, platter.C);
...
*/
    ...
    -8, 8 => begin
                ...
                um.next-array := id + 1;
                um.arrays[id] := make(<scroll>, size: as(<integer>, platter.C), fill: 0);
                platter.B := as(<machine-word>, id);
             end;
/*
           #9. Abandonment.
...
*/
    -4, 12 => begin
                ...
                um.execution-finger := as(<integer>, platter.C);
                um.scroll := shallow-copy(get-array(as(<integer>, platter.B)));
              end;
...
*/
    -3, 13 => platter.literal-A := %logand(platter, #x01FFFFFF); // u%shift-left(1, 25) - 1
  ...
  exception (e :: <error>)
    ...
    // report-condition(e, *standard-output*); // for D2C
    // report-condition(e); // for OD
    // condition-force-output(*standard-output*); // for D2C?
    format-out("\n");
  end block;
